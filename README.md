...
На данный момент язык полностью ушёл под контроль @TISERZ_DEVPROD, и сейчас находится на масштабной переделке, так как сейчас язык является proof-of-concept, и не может в данный момент конкурировать с остальными языками из-за своей структуры, на данный момент новая структура готова на 80%
